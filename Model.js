...
var RUNTIMES = {
  vllm: {
    ...
    // readSample return null -- there was a `noActivity: true` flag here as
...
function fleetState(nodes) {
  var up = 0, down = 0, unread = 0, active = 0, unknown = 0, tokens = 0, running = 0, waiting = 0
...
function columnValues(nodes, field) {
  ...
  for (var i = 0; i < rows.length; i++) {
    if (field === "label") out.push(String(rows[i].label || ""))
      ...
      var rt = rows[i].runtime ? runtimeOf(rows[i].runtime) : null
...
// ── Polling decisions ─────────────────────────────────────────────────
//
// The three pure decisions behind Service.qml's polling loop. They live here,
// executed by tests, because the last review found the fleet-wide version of
// the first one -- the fence that decides whether a host may be polled -- was
// asserted only by matching QML source text, and the defect it was written to
// prevent had inverted underneath the assertion without breaking it.
...
function tableRows(servers, rows, state) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    out.push(have[host] || blankNode(host, list[i].label, cached[host]))
  }
