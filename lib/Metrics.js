...
// endpoint.
//
// Chosen by whoever runs the server, so treated like any other remote value:
// tail first, then stripped and clamped, before it reaches the shared bar.
...
//
// Summing is right for COUNTS -- two engines each running one request really
// are two requests -- and wrong for a FRACTION: two engines each 50% full sum
// to 1, which renders as "100% cache". That is the "is it struggling" number,
...
function avgMetric(text, metric) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // turns "0.5 1700000000000" into "170000000000000% cache".
...
function readSample(runtimeName, body) {
  ...
  if (runtimeName === "ollama") {
    ...
    // No model resident is a CONCLUSIVE answer: nothing loaded cannot be
    // generating. An empty string here is falsy and reads as "no signal", which
    // throws that away. The sentinel cannot collide with a real token, which
    // always ends in ";".
  ...
  // dereferences it, on any body containing a line starting with "null".
...
//
// Comparing the model list for INEQUALITY reads every change as work --
// including a model evicted for inactivity, which is the most conclusive
// evidence available that none happened. A model that was not there before is
// not evidence either: it was loaded, not necessarily used. Only a model in
// BOTH samples whose expiry moved forward says anything.
//
// Compared as parsed times, not strings: the fractional-second field varies in
// length, and a shorter fraction that is a prefix of a longer one sorts
// backwards because "Z" outranks every digit. An unorderable pair claims
// nothing -- it can withhold a "working", never invent one.
...
//
// The two must not share a shape: `{ active: false }` renders as "idle", so a
// counter that RESET (an engine restart) or that the exporter omitted last poll
// would draw green "idle" over a node serving four requests. Both are the case
// this function cannot answer; the caller turns null into "measuring".
