...
//
// Its output is interpolated into a shell command, so isSafeHost is the
// boundary: anything that could end an argument or start another command must
// be impossible to express. A rejected entry is REPORTED, never dropped -- a
// silent drop just shortens the fleet with nothing to explain it.

// Parse the `servers` setting into { host, label } pairs.
//
// `10.0.0.5 = DGX Spark, gpu.local:8000` -- an entry may name its host or be a
// bare one. The separator is the COMMA, because a nickname may contain spaces;
// a bare entry is also split on whitespace so the older "a b" form still works.
// A hostname cannot contain "=", so the split is unambiguous.
...
//
// The settings label, the manifest schema and the panel's help text all
// advertise `host:port`. Without this, discovery appends its sweep to whatever
// it was given and probes "http://gpu.local:8000:8000/metrics".
